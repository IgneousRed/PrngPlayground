...
pub const Seeder = struct {
    ...
    pub fn next(self: *Self) Out {
        var t = self.state;
        var value: [4]u64 = undefined;
        for (value) |*s| {
            s.* = @truncate(u64, t);
            t = t >> 64;
        }
        var r = SeederHash(u64, value);
        var result: Out = 0;
        for (r) |q| {
            result = (result << 64) | q;
        }
...
fn SeederHash(comptime T: type, state: [4]T) [4]T {
    var result: [4]T = undefined;

    // Init words
    for (result) |*s, i| {
        s.* = dev.oddPhiFraction(T) +% state[i];
    }
    // Mix words
    var i: i64 = 0;
    while (i < 1 << 3) {
        var j = @intCast(u64, i) % 4;
        result[j] -%= result[(@intCast(u64, i) + 3) % 4] *% dev.harmonicLCG(u64);
        result[j] ^= result[j] >> 25;
        i +%= 1;
    }
    i = 0;
    while (i < 2) {
        var j = @intCast(u64, i) % 4;
        result[j] *%= dev.harmonicMCG(u64);
        result[j] ^= result[j] >> 25;
        i +%= 1;
    }
    return result;
